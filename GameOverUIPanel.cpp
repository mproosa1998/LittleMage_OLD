...
GameOverUIPanel::GameOverUIPanel(Vector2i res) :UIPanel(res, (res.x / 10) * 3, res.y / 2, (res.x / 10) * 3, res.y / 6, 25, 255, 255, 255) {
	m_ButtonWidth = res.x / 20;
	m_ButtonHeight = res.y / 20;
	m_ButtonPadding = res.x / 100;

	m_Text.setFillColor(sf::Color(0, 255, 255, 0));
...
void GameOverUIPanel::initialiseButtons() {
	addButton(m_ButtonPadding, m_ButtonPadding, m_ButtonWidth, m_ButtonHeight, 0, 255, 0, "Continue?");
